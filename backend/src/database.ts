...
const ensureDirectoryRecord = (name: string) => {
  ...
  database.run('INSERT INTO directories (id, name, created_at, updated_at) VALUES (?, ?, ?, ?)', [randomUUID(), normalized, now, now]);
  return true;
};
...
export const listDirectories = (): Directory[] => queryAll<DirectoryRow>(`
  ...
  FROM directories d
  ORDER BY d.created_at, d.name COLLATE NOCASE
`).map(toDirectory);
...
export const updateDirectory = (id: string, name: string): Directory | undefined => {
  const existing = queryOne<DirectoryRow>(`
    SELECT id, name, created_at, updated_at, 0 AS function_count FROM directories WHERE id = ?
  `, [id]);
...
export const importFunctions = (items: FunctionConfigInput[], mode: 'merge' | 'replace', directories: Array<{ name: string }> = []) => {
  database.run('BEGIN TRANSACTION');
  try {
    if (mode === 'replace') database.run('DELETE FROM functions');
    for (const directory of directories) ensureDirectoryRecord(directory.name);
